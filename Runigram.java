...
public class Runigram {
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		double xRatio = (double) image[0].length / width;
    	double yRatio = (double) image.length / height;

		for(int i=0; i<height; i++){
			for(int j=0; j < width; j++){
				int origX = (int) (j * xRatio);
            int origY = (int) (i * yRatio);
            scaledImage[i][j] = image[origY][origX];
			}
		}
		return scaledImage; 
	}
